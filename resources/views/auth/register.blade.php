...
<div id="register-app">
    ...
    <form id="register-form" class="space-y-4">
        <div class="form-control">
            ...
            <input type="text" name="name" id="name-input" placeholder="John Doe" class="input input-bordered w-full" value="{{ $prefillName ?? '' }}" required>
            <label class="label hidden" id="name-error">
                <span class="label-text-alt text-error">Please enter a valid name (e.g., John Snow). Only letters are allowed.</span>
            </label>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('register-form').addEventListener('submit', async function(e) {
        ...
        if (!nameRegex.test(name) || !name.trim()) {
            ...
            nameError.classList.remove('hidden');
            showError('Please enter a valid name (e.g., John Snow). Only letters are allowed.');
            return;
        }
