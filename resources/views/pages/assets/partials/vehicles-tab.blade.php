...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('assets.create', ['category' => 'vehicle']) }}" class="btn btn-primary btn-sm gap-2">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        @else
                            <div class="w-12 h-12 rounded-lg bg-secondary/10 flex items-center justify-center flex-shrink-0">
                                <span class="icon-[tabler--car] size-6 text-secondary"></span>
                            </div>
                        @endif
                        <div>
                            ...
                            @if($asset->vehicle_description)
                                <p class="text-sm text-base-content/60">{{ $asset->vehicle_description }}</p>
                            @endif
                            <div class="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm text-base-content/60">
                                @if($asset->vehicle_ownership)
                                    <span>{{ $asset->vehicle_ownership_name }}</span>
                                @endif
                                @if($asset->license_plate)
                                    <span>Plate: {{ $asset->license_plate }}</span>
                                @endif
                                @if($asset->mileage)
                                    <span>{{ number_format($asset->mileage) }} miles</span>
                                @endif
                            </div>
...
@else
    <div class="text-center py-12 text-base-content/60">
        <span class="icon-[tabler--car-off] size-16 opacity-30"></span>
        <p class="mt-4 text-lg font-medium">No vehicles</p>
        <p class="text-sm">Add your first vehicle to start tracking your automobiles</p>
        <a href="{{ route('assets.create', ['category' => 'vehicle']) }}" class="btn btn-primary mt-4">
            <span class="icon-[tabler--plus] size-4"></span>
            Add Vehicle
        </a>
    </div>
