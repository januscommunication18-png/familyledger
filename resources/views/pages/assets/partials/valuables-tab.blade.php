...
<div class="flex justify-between items-center mb-6">
    <h2 class="text-lg font-semibold">Valuables & Collectables</h2>
    <a href="{{ route('assets.create', ['category' => 'valuable']) }}" class="btn btn-primary btn-sm gap-2">
        <span class="icon-[tabler--plus] size-4"></span>
        Add Valuable
    </a>
</div>

@if($valuableAssets->count() > 0)
    <div class="grid gap-4">
        @foreach($valuableAssets as $asset)
                ...
                <div class="flex items-start justify-between">
                    <div class="flex items-start gap-4">
                        <div class="w-12 h-12 rounded-lg bg-accent/10 flex items-center justify-center flex-shrink-0">
                            <span class="icon-[tabler--diamond] size-6 text-accent"></span>
                        </div>
                        <div>
                            ...
                            <p class="text-sm text-base-content/60">{{ $asset->type_name }}</p>
                            <div class="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm text-base-content/60">
                                @if($asset->collectable_category)
                                    <span>{{ $asset->collectable_category_name }}</span>
                                @endif
                                @if($asset->condition)
                                    <span>Condition: {{ $asset->condition_name }}</span>
                                @endif
                            </div>
                            @if($asset->appraisal_value && $asset->appraisal_date)
                                <p class="text-sm text-base-content/60 mt-1">
                                    Appraised: ${{ number_format($asset->appraisal_value, 2) }} ({{ $asset->appraisal_date->format('M Y') }})
                                </p>
                            @endif
...
@else
    <div class="text-center py-12 text-base-content/60">
        <span class="icon-[tabler--diamond-off] size-16 opacity-30"></span>
        <p class="mt-4 text-lg font-medium">No valuables</p>
        <p class="text-sm">Add your first valuable item or collectable</p>
        <a href="{{ route('assets.create', ['category' => 'valuable']) }}" class="btn btn-primary mt-4">
            <span class="icon-[tabler--plus] size-4"></span>
            Add Valuable
        </a>
    </div>
