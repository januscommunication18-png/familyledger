...
@section('content')
    <div class="max-w-3xl">
        ...
        <form method="POST" action="{{ route('backoffice.discount-codes.store') }}" class="space-y-6" x-data="discountForm()">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="both" {{ old('plan_type', 'both') === 'both' ? 'selected' : '' }}>Both</option>
                        </select>
                        @error('plan_type')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        >
                            ...
                            @foreach ($packagePlans as $plan)
                                <option value="{{ $plan->id }}" data-monthly="{{ $plan->cost_per_month }}" data-yearly="{{ $plan->cost_per_year }}" {{ old('package_plan_id') == $plan->id ? 'selected' : '' }}>
    ...
    <script>
        function discountForm() {
            return {
                ...
                planType: '{{ old('plan_type', 'both') }}',
                selectedPlanId: '{{ old('package_plan_id') }}',
